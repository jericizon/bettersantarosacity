...
import { toSourceReferences } from '~/utils/source'
import { LAW_TYPE_LABEL, LAW_TYPE_PLURAL, LAW_TYPE_BADGE, LAW_BASED_ON } from '~/utils/law'
...
const sourceRefs: SourceReference[] = law ? toSourceReferences(law.source) : []
  ...
  <div v-if="law" data-pagefind-filter="type:laws" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 flex flex-col gap-10">
    ...
    <section aria-labelledby="summary-heading" class="space-y-4">
      ...
      <div class="rounded-lg border border-charcoal/10 bg-white p-5 shadow-sm">
        ...
        <p class="mt-4 text-xs leading-relaxed text-charcoal/70">
    ...
    <section aria-labelledby="details-heading" class="space-y-4">
      ...
      <dl class="grid gap-4 rounded-lg border border-charcoal/10 bg-white p-5 shadow-sm sm:grid-cols-3">
        <div>
          <dt class="text-xs font-semibold uppercase tracking-wide text-charcoal/70">Type</dt>
          <dd class="mt-1 text-sm font-medium text-charcoal">{{ typeLabel }}</dd>
        </div>
        <div>
          <dt class="text-xs font-semibold uppercase tracking-wide text-charcoal/70">Number</dt>
          <dd class="mt-1 text-sm font-medium text-charcoal">{{ law.number }}</dd>
        </div>
        <div>
          <dt class="text-xs font-semibold uppercase tracking-wide text-charcoal/70">Date</dt>
      ...
      <div class="rounded-lg border border-charcoal/10 bg-white p-5 shadow-sm">
        <template v-if="law.documentUrl">
          <a
            ...
            class="inline-block rounded-sm text-sm font-semibold text-rose-accent-dark hover:underline focus-visible:outline focus-visible:outline-2 focus-visible:outline-laguna-green"
    ...
    <section aria-labelledby="related-heading" class="space-y-4">
      ...
      <ul v-if="relatedLaws.length" class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        >
          ...
          <time :datetime="r.date" class="mt-1 text-[11px] text-charcoal/80">{{ r.date }}</time>
        </li>
      </ul>
